...
@RabbitListener(
        ...
        @Queue(RabbitMQConfigTopic.QUEUE_TOPIC_ONE),
...
public class TopicConsumerOne {
    ...
    public void executor(@Payload User msg, Channel channel, Message message, @Headers Map headers) throws IOException, InterruptedException {
        log.info("收到消息{} 队列 {} rouking >>> {} = {}", msg, headers.get(AmqpHeaders.CONSUMER_QUEUE), RabbitMQConfigTopic.ROUTING_KEY_ONE, msg.getUsername());
